...
async function main() {
  ...
  const VIDEO_ID = 'mCj0u8sjWYs'
  const title = 'Statement on Full Registration'
  const slug = 'statement-on-full-registration'

  const content = `<div style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;border-radius:8px;margin-bottom:1.5rem;">
  <iframe
...
<p style="margin-bottom:1.25rem;">Watch the official statement from PM Party leadership on the full registration of the People's Movement Party by the Office of the Registrar of Political Parties (ORPP).</p>
  ...
  if (existing) {
    const updated = await prisma.article.update({
      ...
      data: {
        title,
        content,
        excerpt: 'Watch the official PM Party statement on receiving full registration from the Office of the Registrar of Political Parties.',
