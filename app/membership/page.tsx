  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-8 sm:py-12">
      ...
      <div className="text-center mb-6">
        <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-2">Membership</h1>
        <div className="w-20 h-1 bg-primary-red mx-auto mb-3"></div>
        <p className="text-gray-500 text-sm sm:text-base">Join People&apos;s Renaissance Movement and be part of the change</p>
      </div>

      {/* ── Step 1: How to Join (compact + shareable) ── */}
      <div ref={shareRef} className="bg-gradient-to-br from-primary-blue to-[#002244] text-white rounded-2xl p-4 mb-5 shadow-lg">
        <div className="flex items-center justify-between gap-3 mb-3">
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold bg-white/20 px-2 py-0.5 rounded-full uppercase tracking-wider shrink-0">Step 1</span>
            <h2 className="text-base font-bold leading-tight">Register via ORPP first</h2>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-2 mb-3">
          <div className="bg-white/10 rounded-xl p-3 border border-white/20">
            <p className="font-bold text-xs mb-1.5">📱 Option A — USSD</p>
            <ol className="text-blue-100 text-xs space-y-0.5 list-decimal list-inside">
              <li>Dial <span className="font-mono font-bold text-white">*509#</span></li>
              <li>Select &ldquo;Register as Party Member&rdquo;</li>
              <li>Party Code: <span className="font-bold text-white">105</span></li>
            </ol>
          </div>
          <div className="bg-white/10 rounded-xl p-3 border border-white/20">
            <p className="font-bold text-xs mb-1.5">🌐 Option B — Website</p>
            <ol className="text-blue-100 text-xs space-y-0.5 list-decimal list-inside">
              <li><span className="font-semibold text-white">ippms.orpp.or.ke</span></li>
        ...
        <div className="bg-white/10 rounded-lg px-3 py-2 border border-white/20 mb-3 text-xs text-blue-100">
          Required by law (Political Parties Act). Your party code is <strong className="text-white">105</strong>.
        </div>

        <div className="flex flex-wrap gap-1.5">
          <p className="w-full text-xs text-blue-300 mb-0.5">Share instructions:</p>
          <button onClick={shareToWhatsApp} className="flex items-center gap-1 bg-green-500 hover:bg-green-600 text-white px-2.5 py-1 rounded-lg text-xs font-semibold transition">
          ...
          <button onClick={shareToX} className="flex items-center gap-1 bg-black hover:bg-gray-800 text-white px-2.5 py-1 rounded-lg text-xs font-semibold transition">
            <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.742l7.73-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
            X
          </button>
          <button onClick={shareToFacebook} className="flex items-center gap-1 bg-[#1877F2] hover:bg-[#166fe5] text-white px-2.5 py-1 rounded-lg text-xs font-semibold transition">
            <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
            Facebook
          </button>
          <button onClick={handleShare} className="flex items-center gap-1 bg-white/20 hover:bg-white/30 text-white px-2.5 py-1 rounded-lg text-xs font-semibold transition">
      ...
      <div id="create-profile" className="mb-6">
        <div className="flex items-center gap-2 mb-4">
          <span className="text-xs font-semibold bg-primary-red/10 text-primary-red px-2 py-0.5 rounded-full uppercase tracking-wider shrink-0">Step 2</span>
          <h2 className="text-base font-bold text-gray-900">Create Your Membership Profile</h2>
        </div>

        {/* Already have a profile? */}
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 flex items-center justify-between mb-4 gap-3">
          <p className="text-sm text-gray-600">Already have a profile?</p>
          <Link
            href="/membership/login"
            className="bg-primary-blue text-white text-xs font-semibold px-3 py-2 rounded-lg hover:bg-[#002244] transition shrink-0"
          >
            Login →
          </Link>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          {(step === 'idle' || step === 'fetching') && (
            <div className="p-5">
              <h3 className="font-bold text-gray-900 mb-1 text-sm">Enter your National ID</h3>
              <p className="text-xs text-gray-500 mb-4">
                We&apos;ll check if your details are already in our system via ORPP.
              </p>
              <form onSubmit={handleCheckId} className="space-y-3">
                <div>
                ...
                <button
                  ...
                  className="w-full bg-primary-blue text-white py-3 rounded-xl font-semibold hover:bg-[#002244] transition disabled:opacity-50 text-sm"
                >
                  {step === 'fetching' ? (
          ...
          {step === 'already-registered' && (
            <div className="p-5 text-center">
              <div className="w-10 h-10 bg-primary-blue/10 rounded-full flex items-center justify-center mx-auto mb-3">
                <svg className="w-5 h-5 text-primary-blue" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
              </div>
              <h3 className="font-bold text-gray-900 mb-2 text-sm">Profile already exists</h3>
            ...
            <div className="p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Confirm details and create your profile</p>
                </div>
              </div>

              {/* Member details preview */}
              <div className="bg-gray-50 rounded-xl p-3 mb-4 grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-sm">
                <div>
                  <span className="text-xs text-gray-400 block">Full Name</span>
                  <span className="font-semibold text-gray-900 text-sm">{orppData.surname} {orppData.otherNames}</span>
                </div>
                <div>
                  <span className="text-xs text-gray-400 block">ID Number</span>
                  <span className="font-semibold text-gray-900 text-sm">{orppData.idNumber}</span>
                </div>
                {orppData.dateOfBirth && (
                  <div>
                    <span className="text-xs text-gray-400 block">Date of Birth</span>
                    <span className="text-gray-700 text-sm">{new Date(orppData.dateOfBirth).toLocaleDateString()}</span>
                  </div>
                )}
                {orppData.gender && (
                  <div>
                    <span className="text-xs text-gray-400 block">Gender</span>
                    <span className="text-gray-700 text-sm">{orppData.gender}</span>
                  </div>
                )}
                {orppData.county && (
                  <div>
                    <span className="text-xs text-gray-400 block">County</span>
                    <span className="text-gray-700 text-sm">{orppData.county}</span>
                  </div>
                )}
                {orppData.constituency && (
                  <div>
                    <span className="text-xs text-gray-400 block">Constituency</span>
                    <span className="text-gray-700 text-sm">{orppData.constituency}</span>
                  </div>
                )}
                {orppData.ward && (
                  <div>
                    <span className="text-xs text-gray-400 block">Ward</span>
                    <span className="text-gray-700 text-sm">{orppData.ward}</span>
                  </div>
                )}
              </div>

              <form onSubmit={handleCreateFromOrpp} className="space-y-3">
                <div>
                ...
                <div className="flex gap-3 pt-1">
                  ...
                  >
                    {step === 'creating' ? 'Creating…' : 'Create Profile'}
                  </button>
                </div>
              </form>
            </div>
              ...
              <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600 mt-1">
                    ID <span className="font-mono font-semibold">{nationalId}</span> hasn&apos;t been synced yet — you may not be registered with ORPP, or your data is still being processed.
                  </p>
                </div>
              </div>

              <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-4 text-xs text-amber-800 space-y-1.5">
                <p><strong>Not yet with ORPP?</strong> Dial <span className="font-mono font-bold">*509#</span> or visit <strong>ippms.orpp.or.ke</strong> — party code <span className="font-bold">105</span>.</p>
                <p><strong>Already registered but data not synced?</strong> You can still create your profile now — it will be verified automatically once your ORPP data arrives.</p>
              </div>

              <div className="flex flex-col gap-2">
                <button
                  onClick={() => setStep('manual-entry')}
                  className="w-full bg-primary-blue text-white py-3 rounded-xl font-semibold hover:bg-[#002244] transition text-sm"
                >
          ...
          {(step === 'manual-entry' || (step === 'creating' && !orppData)) && (
            <div className="p-5">
              <h3 className="font-bold text-gray-900 text-sm mb-1">Create Profile Anyway</h3>
              <p className="text-xs text-gray-500 mb-1">
                Profile will be created with <strong>unsynced status</strong> and updated automatically once your ORPP data is confirmed.
              </p>
              <div className="bg-blue-50 border border-blue-200 rounded-xl px-3 py-2 text-xs text-blue-800 mb-4">
                ID: <span className="font-mono font-bold">{nationalId}</span> · Status: <span className="font-semibold text-amber-700">Pending ORPP sync</span>
              </div>
          ...
          {step === 'done' && (
            <div className="p-5 text-center">
              ...
              <h3 className="text-lg font-bold text-gray-900 mb-2">Profile Created!</h3>
              <p className="text-gray-600 text-sm mb-4">{createResult?.message}</p>
              {createResult?.tempPassword && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-3 mb-4 text-sm">
              ...
              <Link
                href="/membership/login"
                className="inline-block bg-primary-blue text-white px-8 py-3 rounded-xl font-semibold hover:bg-[#002244] transition text-sm"
              >
                Go to Login
              </Link>
            </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-6">
        <h2 className="text-base font-bold text-gray-900 mb-1">Membership Categories</h2>
        <p className="text-xs text-gray-500 mb-4">Choose the category that fits your level of commitment.</p>
        <div className="overflow-x-auto -mx-5 px-5">
          <table className="w-full text-xs min-w-[320px]">
            <thead>
              <tr className="bg-primary-blue text-white">
                <th className="px-3 py-2.5 text-left rounded-tl-lg">Type</th>
                <th className="px-3 py-2.5 text-right">Fees (KShs)</th>
                <th className="px-3 py-2.5 text-left rounded-tr-lg">Timeline</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {[
                ...
                <tr key={type} className="hover:bg-gray-50">
                  <td className="px-3 py-2.5 font-medium text-gray-900">{type}</td>
                  <td className="px-3 py-2.5 text-right text-gray-700">{fee}</td>
                  <td className="px-3 py-2.5 text-gray-500">{timeline === 0 ? 'One-Off' : `Every ${timeline} yrs`}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <div className="mt-3 bg-amber-50 border-l-4 border-amber-400 px-3 py-2.5 rounded-r-lg text-xs text-amber-800">
      ...
      <div className="text-center">
        <Link href="/volunteer" className="text-xs text-gray-500 hover:text-primary-blue transition">
